...
namespace Blackberry {
    ...
    void Logger::Init() {
        ...
        // what symbols in pattern mean: 
        // %H = hours, %M = minutes, %S = seconds,
        // %n = name of logger, %t = current thread id, %v = message
        // %l = level, %$ = colored text

		// logSinks[0]->set_pattern("%^[%T] %n: %v%$");
        logSinks[0]->set_pattern("[%H:%M:%S] [%n, thread %t]: %v%$");
		logSinks[1]->set_pattern("[%H:%M:%S, %l] [%n, thread %t]: %v");
